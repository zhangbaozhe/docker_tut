...
public:
  ...
  void updateNonInertial(const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> a, 
      const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> omega, 
      const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> beta)
  {
    mpc_wrapper_.setNonInertial(a, omega, beta);
  }
  void updateModulation(const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> m_pd, 
      const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> m_vd, 
      const Eigen::Ref<const Eigen::Matrix<T, 4, 1>> m_qd)
  {
    mpc_wrapper_.setModulation(m_pd, m_vd, m_qd);
  }
