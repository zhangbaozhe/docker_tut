...
class MpcWrapper
{
 public:
  ...
  bool setOnlineData(const Eigen::Ref<const Eigen::Matrix<T, kOdSize, 1>> od);
  bool setNonInertial(const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> a, 
      const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> omega, 
      const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> beta);
  bool setModulation(const Eigen::Ref<const Eigen::Matrix<T, 3, 1>> m_pd, 
 ...
 private:
  ...
  const T dt_{0.1};
  const Eigen::Matrix<T, kInputSize, 1> kHoverInput_ =
    (Eigen::Matrix<T, kInputSize, 1>() << 9.81, 0.0, 0.0, 0.0).finished();
};
